...
**Estado: gameplay completo + meta-game + progressão adaptativa C1/C3 + câmera +
XP→Supabase + leaderboard global + overhaul de power-ups/hazards (sem stack, gating,
morte no switch, sem inventário) + QoL de editor — tudo validado/commitado (`907efca`).
Antes de novas frentes: aplicar as pendências de Unity (gaps/cooldowns nos assets,
binding do TimeFreeze, decoyCoinChance). Próximo: Economia Fase A (currencies) OU
Control Panel v2/v3 OU Camada 2 (head start).**
